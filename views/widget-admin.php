...
?>
<div class="uploader">
	<input type="submit" class="button" name="<?php echo $this->get_field_name('uploader_button'); ?>" id="<?php echo $this->get_field_id('uploader_button'); ?>" value="<?php _e('Select an Image', 'image_card_widget'); ?>" onclick="imageWidget.uploader( '<?php echo $this->id; ?>', '<?php echo $id_prefix; ?>' ); return false;" />
...
<div id="<?php echo $this->get_field_id('fields'); ?>" <?php if ( empty($instance['attachment_id']) && empty($instance['imageurl']) ) { ?>style="display:none;"<?php } ?>>
	<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title', 'image_card_widget'); ?>:</label>
	...
	<p><label for="<?php echo $this->get_field_id('alt'); ?>"><?php _e('Alternate Text', 'image_card_widget'); ?>:</label>
	...
	<p><label for="<?php echo $this->get_field_id('rel'); ?>"><?php _e('Related', 'image_card_widget'); ?>:</label>
		...
		<span class="description"><?php _e('A recommended HTML5 related terms list is availlable <a href="http://microformats.org/wiki/existing-rel-values#HTML5_link_type_extensions" target="_blank">here</a>.', 'image_card_widget'); ?></span></p>

	<p><label for="<?php echo $this->get_field_id('description'); ?>"><?php _e('Caption', 'image_card_widget'); ?>:</label>
	...
	<p><label for="<?php echo $this->get_field_id('type_slug'); ?>"><?php _e('Type Slug', 'image_card_widget'); ?>:</label>
		<input class="widefat" id="<?php echo $this->get_field_id('type_slug'); ?>" name="<?php echo $this->get_field_name('type_slug'); ?>" type="text" value="<?php echo esc_attr(strip_tags($instance['type_slug'])); ?>" /></p>

	<p><label for="<?php echo $this->get_field_id('link'); ?>"><?php _e('Link', 'image_card_widget'); ?>:</label>
	...
	<label for="<?php echo $this->get_field_id('linkid'); ?>"><?php _e('Link ID', 'image_card_widget'); ?>:</label>
	...
	<select name="<?php echo $this->get_field_name('linktarget'); ?>" id="<?php echo $this->get_field_id('linktarget'); ?>">
		<option value="_self"<?php selected( $instance['linktarget'], '_self' ); ?>><?php _e('Stay in Window', 'image_card_widget'); ?></option>
		<option value="_blank"<?php selected( $instance['linktarget'], '_blank' ); ?>><?php _e('Open New Window', 'image_card_widget'); ?></option>
	</select></p>
	...
	<div id="<?php echo $this->get_field_id('custom_size_selector'); ?>" <?php if ( empty($instance['attachment_id']) && !empty($instance['imageurl']) ) { $instance['size'] = self::CUSTOM_IMAGE_SIZE_SLUG; ?>style="display:none;"<?php } ?>>
		<p><label for="<?php echo $this->get_field_id('size'); ?>"><?php _e('Size', 'image_card_widget'); ?>:</label>
			<select name="<?php echo $this->get_field_name('size'); ?>" id="<?php echo $this->get_field_id('size'); ?>" onChange="imageWidget.toggleSizes( '<?php echo $this->id; ?>', '<?php echo $id_prefix; ?>' );">
				...
				$possible_sizes = apply_filters( 'image_size_names_choose', array(
					'full'      => __('Full Size', 'image_card_widget'),
					'thumbnail' => __('Thumbnail', 'image_card_widget'),
					'medium'    => __('Medium', 'image_card_widget'),
					'large'     => __('Large', 'image_card_widget'),
				) );
				$possible_sizes[self::CUSTOM_IMAGE_SIZE_SLUG] = __('Custom', 'image_card_widget');
	...
	<div id="<?php echo $this->get_field_id('custom_size_fields'); ?>" <?php if ( empty($instance['size']) || $instance['size']!=self::CUSTOM_IMAGE_SIZE_SLUG ) { ?>style="display:none;"<?php } ?>>
		...
		<p><label for="<?php echo $this->get_field_id('width'); ?>"><?php _e('Width', 'image_card_widget'); ?>:</label>
		...
		<p><label for="<?php echo $this->get_field_id('height'); ?>"><?php _e('Height', 'image_card_widget'); ?>:</label>
	...
	<p><label for="<?php echo $this->get_field_id('align'); ?>"><?php _e('Align', 'image_card_widget'); ?>:</label>
	<select name="<?php echo $this->get_field_name('align'); ?>" id="<?php echo $this->get_field_id('align'); ?>">
		<option value="none"<?php selected( $instance['align'], 'none' ); ?>><?php _e('none', 'image_card_widget'); ?></option>
		<option value="left"<?php selected( $instance['align'], 'left' ); ?>><?php _e('left', 'image_card_widget'); ?></option>
		<option value="center"<?php selected( $instance['align'], 'center' ); ?>><?php _e('center', 'image_card_widget'); ?></option>
		<option value="right"<?php selected( $instance['align'], 'right' ); ?>><?php _e('right', 'image_card_widget'); ?></option>
	</select></p>
</div>
