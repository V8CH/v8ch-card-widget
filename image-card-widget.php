<?php
/*
Plugin Name: Image Card Widget
Plugin URI: http://github.com/V8CH/wp-plugin-image-card-widget.git
Description: Extended from the Image Widget from Modern Tribe, with modifications for V8CH clients.
Author: V8CH
Version: 0.1.0
...
function v8ch_load_image_widget() {
	register_widget('V8CH_Image_Card_Widget');
}
add_action('widgets_init', 'v8ch_load_image_widget');

/**
 * V8CH_Image_Card_Widget class
 **/
class V8CH_Image_Card_Widget extends WP_Widget {
	...
	public function __construct() {
		load_plugin_textdomain( 'image_card_widget', false, trailingslashit(basename(dirname(__FILE__))) . 'lang/');
		$widget_ops = array( 'classname' => 'widget_image_card', 'description' => __( 'Showcase a single image with a Title, URL, Description and three posts from a selectable taxonomy', 'image_card_widget' ) );
		$control_ops = array( 'id_base' => 'widget_image_card' );
		parent::__construct('widget_image_card', __('Image Card Widget', 'image_card_widget'), $widget_ops, $control_ops);
	...
	public function admin_setup() {
		...
		wp_localize_script( 'tribe-image-widget', 'TribeImageWidget', array(
			'frame_title' => __( 'Select an Image', 'image_card_widget' ),
			'button_title' => __( 'Insert Into Widget', 'image_card_widget' ),
		) );
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( !empty( $instance['imageurl'] ) || !empty( $instance['attachment_id'] ) ) {
			...
			$instance['type_slug'] = apply_filters( 'widget_type_slug', $instance['type_slug'], $args, $instance );
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['type_slug'] = strip_tags($new_instance['type_slug']);
